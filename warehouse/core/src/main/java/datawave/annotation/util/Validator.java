...
/** A class for defining logic the object validator pattern, rules are added by passing predicates to
 * {@link #addCheck(Predicate, String)} and object instances are validated using {@link #check(Object)}.
 * <p>
 *     validators should be created once and reused. state is externalized in {@link ValidationState}, so
 *     it is safe to use a single instance across multiple threads, but initialization is not thread-safe.
 * </p>
...
public class Validator<T> {
     ...
     *  </pre>
     * @return a validator
     * @param <V> the type of object that will be validated
     */
     ...
     * @param errorMessage the error message to store if validation fails
