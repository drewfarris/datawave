...
public class AccumuloAnnotationSerializer implements AnnotationSerializer<List<Map.Entry<Key,Value>>> {
    ...
    public Annotation deserialize(List<Map.Entry<Key,Value>> elements) throws AnnotationSerializationException {
        ...
        for (Map.Entry<Key,Value> e : elements) {
            ...
            if (cqParts.length == 3) {
                ...
                try {
                    segments.add(Segment.newBuilder().setSegmentId(UID.parse(cqParts[1])).setSegmentData(SegmentData.parseFrom(e.getValue().get())).build());
            ...
            annotationBuilder.setShard(key.getRow().toString()).setDataType(cfParts[0]).setUid(UID.parse(cfParts[1])).setAnnotationType(cfParts[2])
    ...
    protected static Map.Entry<Key,Value> serializeMetadata(Key baseKey, UID annotationId, String metadataKey, String metadataValue) {
        ...
        final Key key = Key.builder().row(baseKey.getRowData().getBackingArray()).family(baseKey.getColumnFamilyData().getBackingArray())
                        .qualifier(columnQualifier).visibility(baseKey.getColumnVisibilityData().getBackingArray()).timestamp(baseKey.getTimestamp()).build();
