// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: AnnotationV1.proto

package datawave.annotation.protobuf.v1;

public interface SegmentOrBuilder extends
    // @@protoc_insertion_point(interface_extends:datawave.annotation.protobuf.v1.Segment)
    com.google.protobuf.MessageOrBuilder {

  /**
   * <pre>
   * the 32-bit hash used for segment identification and grouping.
   * </pre>
   *
   * <code>string segmentHash = 1;</code>
   * @return The segmentHash.
   */
  java.lang.String getSegmentHash();
  /**
   * <pre>
   * the 32-bit hash used for segment identification and grouping.
   * </pre>
   *
   * <code>string segmentHash = 1;</code>
   * @return The bytes for segmentHash.
   */
  com.google.protobuf.ByteString
      getSegmentHashBytes();

  /**
   * <pre>
   * the boundary for this segment.
   * </pre>
   *
   * <code>.datawave.annotation.protobuf.v1.SegmentBoundary boundary = 2;</code>
   * @return Whether the boundary field is set.
   */
  boolean hasBoundary();
  /**
   * <pre>
   * the boundary for this segment.
   * </pre>
   *
   * <code>.datawave.annotation.protobuf.v1.SegmentBoundary boundary = 2;</code>
   * @return The boundary.
   */
  datawave.annotation.protobuf.v1.SegmentBoundary getBoundary();
  /**
   * <pre>
   * the boundary for this segment.
   * </pre>
   *
   * <code>.datawave.annotation.protobuf.v1.SegmentBoundary boundary = 2;</code>
   */
  datawave.annotation.protobuf.v1.SegmentBoundaryOrBuilder getBoundaryOrBuilder();

  /**
   * <pre>
   * multiple values associated with this segment.
   * </pre>
   *
   * <code>repeated .datawave.annotation.protobuf.v1.SegmentValue values = 3;</code>
   */
  java.util.List<datawave.annotation.protobuf.v1.SegmentValue> 
      getValuesList();
  /**
   * <pre>
   * multiple values associated with this segment.
   * </pre>
   *
   * <code>repeated .datawave.annotation.protobuf.v1.SegmentValue values = 3;</code>
   */
  datawave.annotation.protobuf.v1.SegmentValue getValues(int index);
  /**
   * <pre>
   * multiple values associated with this segment.
   * </pre>
   *
   * <code>repeated .datawave.annotation.protobuf.v1.SegmentValue values = 3;</code>
   */
  int getValuesCount();
  /**
   * <pre>
   * multiple values associated with this segment.
   * </pre>
   *
   * <code>repeated .datawave.annotation.protobuf.v1.SegmentValue values = 3;</code>
   */
  java.util.List<? extends datawave.annotation.protobuf.v1.SegmentValueOrBuilder> 
      getValuesOrBuilderList();
  /**
   * <pre>
   * multiple values associated with this segment.
   * </pre>
   *
   * <code>repeated .datawave.annotation.protobuf.v1.SegmentValue values = 3;</code>
   */
  datawave.annotation.protobuf.v1.SegmentValueOrBuilder getValuesOrBuilder(
      int index);

  /**
   * <pre>
   * optional segment metadata
   * </pre>
   *
   * <code>map&lt;string, string&gt; metadata = 50;</code>
   */
  int getMetadataCount();
  /**
   * <pre>
   * optional segment metadata
   * </pre>
   *
   * <code>map&lt;string, string&gt; metadata = 50;</code>
   */
  boolean containsMetadata(
      java.lang.String key);
  /**
   * Use {@link #getMetadataMap()} instead.
   */
  @java.lang.Deprecated
  java.util.Map<java.lang.String, java.lang.String>
  getMetadata();
  /**
   * <pre>
   * optional segment metadata
   * </pre>
   *
   * <code>map&lt;string, string&gt; metadata = 50;</code>
   */
  java.util.Map<java.lang.String, java.lang.String>
  getMetadataMap();
  /**
   * <pre>
   * optional segment metadata
   * </pre>
   *
   * <code>map&lt;string, string&gt; metadata = 50;</code>
   */

  java.lang.String getMetadataOrDefault(
      java.lang.String key,
      java.lang.String defaultValue);
  /**
   * <pre>
   * optional segment metadata
   * </pre>
   *
   * <code>map&lt;string, string&gt; metadata = 50;</code>
   */

  java.lang.String getMetadataOrThrow(
      java.lang.String key);
}
