...
/** This class represents a specific phrase that has been matched in a retrieved document field. It contains the
 *  id of the event it is associated with and the start and end offset of the phrase in the field specified. The
 *  offsets match those found for individual terms in the tf portion of the index.
 *
...
public class PhraseOffset implements Comparable<PhraseOffset> {
    ...
    public boolean equals(Object o) {
        ...
        return new EqualsBuilder().append(startOffset, that.startOffset).append(endOffset, that.endOffset).append(eventId, that.eventId).isEquals();
    }

    @Override
    public int hashCode() {
        return new HashCodeBuilder(17, 37).append(eventId).append(startOffset).append(endOffset).toHashCode();
    }
}
