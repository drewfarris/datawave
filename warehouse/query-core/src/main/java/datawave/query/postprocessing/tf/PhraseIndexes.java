...
public class PhraseIndexes {
     ...
     * <li>Given {@code BODY:event1,1,2:event2,3,5/CONTENT:event3,5,6:event4,7,6}, a {@link PhraseIndexes} will be 
     * returned with offsets [1,2] for event1 and [3,5] for event2 in the field {@code BODY}, 
     * and offsets [5,6] in event3 and [7,6] for event4 and field {@code CONTENT}.</li>
     * </ul>
     *
     ...
     */
    public static PhraseIndexes from(final String phraseIndexString) {
        if (phraseIndexString == null) {
            return null;
        }

        // Strip whitespaces.
        final String cleanPhraseIndexString = StringUtils.deleteWhitespace(phraseIndexString);

        PhraseIndexes phraseIndexes = new PhraseIndexes();
        final String[] fieldParts = cleanPhraseIndexString.split(Constants.FORWARD_SLASH);
     ...
     * @param field
     *            the field
     * @return the offsets if any, otherwise an empty collection or null
     */
