...
public class MutableMetadataHandler extends ModificationServiceConfiguration {
            ...
            if (key.getType().equals(KeyType.INDEX_EVENT)) {
                ...
                try {
                    for (Key fiKey : fiKeys) {
                        ...
                        TermFrequencyIterable tfKeys = getTermFrequencyKeys(client, this.getEventTableName(), userAuths, shardId, fiKeyParsed.getDataType(),
                                        fiKeyParsed.getUid(), fiKeyParsed.getFieldValue(), fiKeyParsed.getFieldName());
                        try {
    ...
    protected static class TermFrequencyIterable implements Iterable<Key>, AutoCloseable {
        ...
        public Iterator<Key> iterator() {
            ...
            }), new Function<Entry<Key,Value>,Key>() {
                @Nullable
                @Override
                public Key apply(@Nullable Entry<Key,Value> keyValueEntry) {
                    return keyValueEntry.getKey();
